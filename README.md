...
| Command | Description |
|---------|-------------|
| `ctrace.runAnalysis` | Run ctrace on the currently active file |

The command can also be triggered programmatically with a parameters string:

```typescript
vscode.commands.executeCommand('ctrace.runAnalysis', '--entry-points=main --static');
```
...
To produce a `.vsix` installable extension:
...
Then install it in any VS Code instance:

```bash
code --install-extension ctrace-audit-0.0.1.vsix
```
